...
# SAME
    ...
    if pm.id in last_threat_lvl and (not lst or (lst and max(lst) < SAME)):
        if last_threat_lvl[pm.id] == RUN:
            for flt in foe_last_turn:
                if dist(flt.pos, pm.pos) <= 4:
                    debug("RUN from prev")
                    lst.append(RUN)
                    accessible = set([a for a in accessible if dist(
                        flt.pos, pm.pos) < dist(a, flt.pos)])
    ...
    if my_close_pacmen:
        for mcp in my_close_pacmen:
            if mcp not in intersect or pm.pos not in intersect:
                if r % 2:
                    if mcp.id - pm.id < 0:
    ...
    if pm.id in last_pos and last_pos[pm.id] == pm.pos and not any([p.pos in diagonals(pm.pos) for p in my_pacmen]):
...
def final_decision(pm, lst):  # choose the one that goes in the longest tunnel, away from my team
    ...
    if len(lst) == 1:
        return rem
    # if any([l in intersect for l in lst]):
    #     debug("target intersect")
    #     inter = 0
    #     for l in lst:
    #         if l in intersect and len([al for al in around(l) if al in to_explore]) > inter:
    #             inter = len([al for al in around(l) if al in to_explore])
    #             rem = l
    elif any([l in pellets or l in to_explore for l in lst]):
        debug("target longest tunnel")
        tunnel_size = 0
        for elem in lst:
            for i, tu in enumerate(tunnels):
                if elem in tu:
                    if len(tu) > tunnel_size and tu not in occupied_tunnels.values():
                        rem = elem
                        tunnel_size = len(tu)
                        occupied_tunnels[pm.id] = tu
    else:
        debug("target short tunnels")
        tunnel_size = width
        for elem in lst:
            for i, tu in enumerate(tunnels):
                if elem in tu:
...
def get_closest(pm):
    ...
    if not res:
        for i in intersect:
            tot = 0
            if dist(i, pm.pos) < distance and i not in targets.values() and not any([(dist(i, o.pos) <= dist(i, pos) and o.pos != pos and (last_pos[o.id] != o.pos)) for o in my_pacmen]) and i != pm.pos:
                for ai in around(i):
                    for tu in tunnels:
                        if ai in tu:
                            if not any([t not in to_explore for t in tu]):
                                tot += len(tu)
            if tot >= mini:
    ...
    if not res:
        ...
        debug("LSG 3")
        for i in intersect:
            # and dist(i, pm.pos) <= dist(i, last_pos[pm.id])
            tot = 0
            if dist(i, pm.pos) < distance and i not in targets.values() and i != pm.pos:
                for ai in around(i):
                    if ai in to_explore:
                        tot += 1
            if dist(i, pm.pos) < distance and tot >= mini:
                distance = dist(i, pm.pos)
                res = i
    if not res:
        res = Coord(0, 0)
...
def choose_direction(pm):  # care not mess up with accessible vs available
    ...
    if pm.id in long_shot:
        # careful about that
        if long_shot[pm.id] == pm.pos or all([a not in to_explore + big_pellets for a in around(long_shot[pm.id])]):
    ...
    if threat_degree == NOTHING or threat_degree == IGNORE or threat_degree == RUN:
        ...
        if pm.id in long_shot and long_shot[pm.id] in big_pellets:
            ...
            if threat_degree == RUN:
                debug(f'accessible = {accessible}')
                if any([dist(a, long_shot[pm.id]) < dist(pm.pos, long_shot[pm.id]) for a in accessible]):
                    accessible = set([a for a in accessible if dist(
                        a, long_shot[pm.id]) < dist(pm.pos, long_shot[pm.id])])
                    debug(f'accessible ls = {accessible}')
            else:
        ...
        if not pm.stl:
            ...
            # D'abord on check pour des pellets
            if any([a in pellets + big_pellets for a in accessible]):
                debug(f" - {pm.id} pellets !")
                accessible = set(
                    [a for a in accessible if a in pellets + big_pellets])
                ...
                if not pm.id in long_shot or long_shot[pm.id] == pm.pos:
                    long_shot[pm.id] = get_closest(pm)
                accessible = set([long_shot[pm.id]])
        else:
            debug(f"stl = {pm.stl}")
            if any(a in pellets + big_pellets + to_explore for a in accessible):
                accessible = set(
                    [a for a in accessible if a in pellets + big_pellets + to_explore])
                debug(f"pellets {accessible}")
            # elif any([any([pos in to_explore and pos in around(pm.pos) for pos in around(a)]) for a in accessible]):
            #     accessible = set([a for a in accessible if any(
            #         [pos in to_explore and pos in around(pm.pos) for pos in around(a)])])
            #     debug(f"explore {accessible}")
            elif threat_degree != RUN:
                if not pm.id in long_shot or long_shot[pm.id] == pm.pos:
                    long_shot[pm.id] = get_closest(pm)
                accessible = set([long_shot[pm.id]])
                debug(f"ls {accessible}")
            if any([dist(a, pm.pos) == 2 for a in accessible]):
                accessible = set(
                    [a for a in list(accessible) if dist(a, pm.pos) == 2])
                debug(f"dist == 2 {accessible}")
            # elif any([any([pos in pellets and pos in around(pm.pos) for pos in around(a)]) for a in accessible]):
            #     accessible = set([a for a in accessible if any(
            #         [pos in pellets and pos in around(pm.pos) for pos in around(a)])])
        if not more_in_c or threat_degree == RUN:
            if any([a not in culsdesacs for a in accessible]):
                accessible = set(
                    [a for a in accessible if a not in culsdesacs])
                debug(f"avoid cds {accessible}")
        else:
            if any([a in culsdesacs for a in accessible]):
                accessible = set(
                    [a for a in accessible if a in culsdesacs])
                debug(f"going in a cds {accessible}")
...
def print_directives(my_pacmen, big_pellets):
    ...
    for pm in my_pacmen:
        if targets[pm.id] in TYPS:
            ...
            commands.append(
                f"MOVE {pm.id} {targets[pm.id].x} {targets[pm.id].y} ^_^")
    print("|".join(commands))
...
def assign_bp():
    ...
    for dup in duplicate_values(closest_big_by_id):
        ...
        if not dup in long_shot.values():
    ...
    for coord in unique_values(closest_big_by_id):
        for pm in my_pacmen:
            if pm.id in closest_big_by_id and closest_big_by_id[pm.id] == coord:
                if not coord in long_shot.values():
