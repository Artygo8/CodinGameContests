...
#### THREAT HANDLING
        if (eval(pm.typ) + 1) % 3 == eval(unit.typ):
            if (dist(unit.pos, pm.pos) == 2 and unit.stl):  # it can eat me anyway
                ...
                if pm.cld == 0 and unit.cld != 0:
                    ...
                    lst.append(THREAT) ### NORMALLY ITS SAME BUT TO BEAT OTHER PLAYERS I WILL TRY THREAT
                else:
                    lst.append(RUN)
                    accessible.discard(unit.pos)
                    for i in around(unit.pos):
    ...
    my_close_pacmen = [m.pos for m in my_pacmen if (dist(m.pos, pm.pos) == 1 or dist(m.pos, pm.pos) == 2) and m.pos in last_pos]
    if my_close_pacmen:
        lst.append(FREEZE)
    if lst:
        return (max(lst))
...
def choose_direction(pm): # care not mess up with accessible vs available
    accessible = get_accessible(pm.pos)  # simpler version of possible as a SET set()
    ...
    if threat_degree == RUN:
        ...
        if any([not min_walls_around(a, 4) for a in accessible]):
            accessible = set([a for a in accessible if not min_walls_around(a, 5)])
        if accessible:
    ...
    if threat_degree == NOTHING or threat_degree == IGNORE:
        ...
        if not pm.stl:
            if any([dist(a, pm.pos) == 1 for a in accessible]):
                accessible = set([a for a in accessible if dist(a, pm.pos) == 1])
            ...
            if any([any([type(grid.get_cell(pos.x, pos.y)) == int for pos in around(a)]) for a in accessible]):
                accessible = set([a for a in accessible if any([type(grid.get_cell(pos.x, pos.y)) == int for pos in around(a)])])
            elif pm.id in long_shot: # si y a pas de pellets, on va vers la destination random
                accessible = set([long_shot[pm.id]])
